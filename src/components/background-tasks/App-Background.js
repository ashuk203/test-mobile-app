...
BackgroundTask.define(async () => {
  // Init queue
  queue.configure({
    onQueueFinish: (executedJobs: Job[]) => {
  ...
  render() {
    return (
      <ScrollView style={styles.container}>
        ...
        {this.state.showImages && (
          <Text style={styles.welcome}>Image Screen</Text>
        )}
        {this.state.showImages && (
          <Image
            style={styles.image}
            source={{uri: 'https://i.imgur.com/kPkQTic.jpg'}}
            permanent={false}
          />
        )}
        {this.state.showImages && (
          <Image
            style={styles.image}
            source={{uri: 'https://i.redd.it/uwvjph19mltz.jpg'}}
            permanent={false}
          />
        )}
        {this.state.showImages && (
          <Image
            style={styles.image}
            source={{uri: 'https://i.redd.it/39w0xd9ersxz.jpg'}}
            permanent={false}
          />
        )}
      </ScrollView>
